...
const swlSuccess = (text: any) => {
  Swal.fire(
    'Categoría '+ text,
    'La categoría ha sido '+ text +' exitosamente',
    'success'
  )
}
const swlErr = () => {
...
const Categorias: FC = () => {
  ...
  const baseUrl = 'https://crisalex.cl/api/?metodo=categorias'
  ...
  const peticionDelete=async()=>{
    ...
    .then(response=>{
      if(isNaN(response.data)){
        return ( Swal.fire('No se puede eliminar la Categoría','La Categoría esta asociada a un Equipo','error'), setModalEliminar(false))
      }
    ...
    <div className={`card mb-5 mb-xl-8`}>
      ...
      <MaterialTable
          ...
          options={{
            ...
            exportMenu: [{
              label: 'Exportar PDF',
              exportFunc: (cols, x) => ExportPdf(cols, data, 'Categorias')
            },{
              label: 'Exportar CSV',
              exportFunc: (cols, x) => ExportCsv(cols, data, 'Categorias')
            }]
          }}
          localization={{
            header:{
